...
def edit_ticket(content):
    ...
    if content['priority']:
        content['limit_date'] = timedelta(days=PRIORITY_AND_DAYS_TO_COMPLETE[int(content['priority'])]) + datetime.strptime(get_ticket_by_id(_id)[0]['creation_date'], "%Y-%m-%d %H:%M:%S.%f")
